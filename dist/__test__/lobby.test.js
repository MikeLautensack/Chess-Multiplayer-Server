...
describe("lobby tests", () => {
    ...
    beforeAll(() => {
        ...
        privateLobby = new Lobby("test private lobby", host, "private", "test password");
    ...
    test("add player", () => {
        ...
        expect(publicLobby.getPlayers().has(host.getUsername())).toBeTruthy();
        expect(publicLobby.getPlayers().has(whitePlayer.getUsername())).toBeTruthy();
        // Private Lobby Assertions
        expect(privateLobby.getPlayers().size).toBe(2);
        expect(privateLobby.getPlayers().has(host.getUsername())).toBeTruthy();
        expect(privateLobby.getPlayers().has(whitePlayer.getUsername())).toBeTruthy();
    });
    test("remove player", () => {
        // Mutations
        publicLobby.addPlayer(whitePlayer);
        ...
        expect(privateLobby.getPlayers().size).toBe(1);
        expect(privateLobby.getPlayers().has(host.getUsername())).toBeTruthy();
        expect(privateLobby.getPlayers().has(whitePlayer.getUsername())).toBeFalsy();
    });
    test("add game", () => {
        // Mutations
