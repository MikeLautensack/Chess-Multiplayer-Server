...
import { files, ranks, startingPositions, } from "../Models/StandardChessBoard.js";
    ...
    test("init standard chess board", () => {
        ...
        for (let i = 0; i < board.getSquares().length; i++) {
            for (let j = 0; j < board.getSquares()[i].length; j++) {
                ...
                expect(board.getSquares()[i][j].getPiece()?.getColor()).toBe(startPos?.pieceColor);
                expect(board.getSquares()[i][j].getPiece()?.getPieceStr()).toBe(startPos?.pieceType);
            }
        }
    });
});
